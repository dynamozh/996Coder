...
discrip:
后端直接使用objects, outimg = run(img_BGR, CountArea)函数获取结果，img_BGR是通过cv2读取成BGR的图片
（见本文件最后main函数），objects是检测出来的信息，形式如：
[['car', 0.539247453212738, [1290, 538, 74, 42]], ['car', 0.6105919480323792, [2034, 438, 65, 43]]]
run(img_BGR, CountArea)函数体中，对加了框的图片进行保存
...
from . import predict, config, models
from .utils import datasets, utils, parse_config

# Yu-cheng Computer import path
# import predict, config, models
# from utils import datasets, utils, parse_config

base_path = Path(__file__).parent

data_config = str((base_path / "./config/coco.data").resolve())
weights_path = str((base_path / "./weights/yolov3.weights").resolve())
model_def = str((base_path / "./config/yolov3.cfg").resolve())
permission = config.names
colorDict = config.color_dict
CountArea = datasets.np.array([[241, 182], [2408, 147], [2454, 1326], [124, 1322]])
data_config = parse_config.parse_data_config(data_config)
yolo_class_names = utils.load_classes('./config/coco.names')
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
yolo_model = models.Darknet(model_def).to(device)
num_dict = {"bicycle": 0, "bus": 0, "car": 0, "motorbike": 0, "truck": 0}


def initModel():
    num_dict["bicycle"] = num_dict["bus"] = num_dict["car"] = num_dict["motorbike"] = num_dict["truck"] = 0
    print("Loading model ...")
...
def get_results(model,device, class_names,permission, frame, CountArea,colorDict):
    ...
    painting = datasets.np.zeros((AreaBound[3] - AreaBound[1], AreaBound[2] - AreaBound[0]), dtype=datasets.np.uint8)
    ...
    #     cv2.putText(frame, str(num_dict[objectName]) + "_" + objectName, (x1 - 1, y1 - module_carDetection), cv2.FONT_HERSHEY_COMPLEX, 0.7,
    #                 boxColor,
    #                 thickness=2)
...
def run(image, CountArea):
    ...
    for item in objects:
        ...
        # save_file_path = os.path.join('out_imgs/', '%s.jpg' % (image_name))  # 本次保存图片jpg路径
    cv2.imwrite('D:\django\car_recognition2\media\images\outimg.jpg', outimg)

    return objects, outimg,num_dict
...
if __name__ == '__main__':
    ...
    plt.title('BGR')
    plt.show()
    the_objects, the_outimg,the_num_dict = run(img_BGR, CountArea)